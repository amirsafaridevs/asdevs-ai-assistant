...
class ChatProxyController
{
    ...
    /**
     * Permission check: only administrators may use the AI proxy.
     */
        ...
        foreach ($lines as $line) {
            ...
            if (isset($delta['tool_calls'])) {
                foreach ($delta['tool_calls'] as $tc) {
                    $fn = $tc['function'] ?? [];
                    if (isset($fn['name'])) {
                        $this->sendSSE('tool_start', wp_json_encode([
                            'id'   => $tc['id'] ?? '',
                            'name' => $fn['name'],
                        ]));
                    }
                    if (isset($fn['arguments'])) {
                        $this->sendSSE('tool_args', wp_json_encode([
                            'id'   => $tc['id'] ?? '',
                            'args' => $fn['arguments'],
                        ]));
                    }
    ...
    private function sendSSE(string $event, string $data): void
    {
        ...
        foreach (explode("\n", $data) as $line) {
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- SSE protocol requires raw output with per-line data fields
            echo 'data: ' . $line . "\n";
        }
